...
import { getBestsellerProducts, getBrandsWithProductCount, getFeaturedProducts, getHighlightedCategories, getLatestBlogPosts, getPopularCars } from "@/lib/data";
...
export default async function Home() {
  const [featuredProducts, bestsellerProducts, categories, brands, cars, posts] = await Promise.all([
  ...
  return (
    <div className="space-y-10 pb-12">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-8">
            {categories.map((category) => (
              ...
              >
                ...
                <span className="line-clamp-2 text-sm font-bold text-[#374151]">{category.name}</span>
                <span className="mt-2 block text-xs font-semibold text-[#6B7280]">{category._count.products} محصول</span>
              </Link>
            ))}
          </div>
        ) : (
      ...
      <section className="container-zen grid gap-5 lg:grid-cols-[1fr_1fr]">
        <div className="rounded-3xl border border-[#E5E7EB] bg-white p-6">
          ...
          {cars.length > 0 ? (
            ...
            <EmptyState message="هنوز دفترچه یا خودرویی ثبت نشده است." compact />
          )}
        </div>
      ...
      {brands.length > 0 && (
        <section className="container-zen space-y-5">
          <SectionHeader title="برندهای محبوب" subtitle="برندهای ثبت‌شده در فروشگاه" href="/brands" />
...
function EmptyState({ message, compact = false }: { message: string; compact?: boolean }) {
  return (
    <div className={`rounded-2xl border border-dashed border-[#D1D5DB] bg-white text-center text-sm font-semibold text-[#6B7280] ${compact ? "mt-5 p-5" : "p-8"}`}>
      {message}
    </div>
  );
}
