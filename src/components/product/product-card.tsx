...
export function ProductCard({ product, compact = false, priority = false }: ProductCardProps) {
  ...
  return (
    <article className="group flex h-full min-w-0 flex-col border-b border-border pb-4">
      {product.imageUrl ? (
        <div className="relative">
          ...
          <Link aria-label={product.name} className="block overflow-hidden rounded-xl bg-surface-secondary" href={`/products/${product.slug}`}>
            <div className="relative aspect-[4/3] sm:aspect-[10/9]">
              <Image
