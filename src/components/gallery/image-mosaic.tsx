  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      {images.slice(0, 3).map((image, index) => (
        <div
          key={image.id}
          className={`group relative overflow-hidden rounded-[28px] border border-white/10 bg-black/40 transition hover:border-purple-400/50 ${
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 flex flex-col gap-2 p-6 text-white">
            ...
            {image.link && (
              <a
                href={image.link}
                className="self-start text-xs font-semibold text-purple-200 hover:text-purple-100"
              >
                مشاهده بیشتر →
              </a>
            )}
