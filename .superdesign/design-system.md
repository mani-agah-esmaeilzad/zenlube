...
## Component rules
- Oil selection guide: an open white, image-led editorial section near the homepage footer, with an educational manual/bottle illustration on the left on desktop and above the text on phones. Keep the technical sample grade explicitly illustrative, use concise unframed viscosity/standards facts, one compact vehicle-manual CTA and a quiet blog link. No numbered mini-cards, large buttons or another dark band.
...
- Header category navigation: use one compact popover surface with open divider-led rows, never a grid of nested cards. Keep categories without active products in the complete directory instead of the purchase menu, render a real image only when one exists, and keep clear text links to the complete category directory and vehicle notebook. On mobile, category rows remain a single column inside the right-side drawer.
- Product card (approved September 12): flat white image-led cards without a gray image frame, nested panels or filled purchase buttons. Use a muted 12px brand, a 15px/mobile–16px/desktop semibold short title, and a separate readable 13px model/pack-size line. Preserve safety-critical oil grades and exact named millilitre volumes. Phones show one full-width horizontal card with the photo on the right; tablet/desktop use vertical cards. One open, divider-led bottom row has a quiet “مشاهده محصول” link on the right and a 20px/mobile–22px/desktop bold Persian price on the left, with a smaller “تومان” unit to the left of the digits. Stored prices stay in rials. Unavailable cards show “ناموجود” and no price. Keep the wishlist as an unframed outline heart and reserve badges/old prices for genuine active discounts; omit ratings and category-label pills. Never include an add-to-cart button.
